...
// 2. CORES E HELPERS (Isto corre sempre bem)
partyColors = {
  return {
...
// 3. CARREGAR DADOS
// O .catch impede que o site crashe se o ficheiro falhar
...
// 4. PREPARAÇÃO DE DADOS (Só corre se os ficheiros existirem)
...
// A. Calcular Vencedores por Distrito
winners = {
  const w = {};
  if (!dadosOK) return w;
...
## Column {width=50%}
```{ojs}
// MAPA 1: VENCEDOR (Versão de Correção)
mapaVencedor = {
  const div = document.createElement("div");
  // ALTERAÇÃO 1: Altura fixa para garantir que aparece
  div.style.height = "500px"; 
  div.style.width = "100%";
  div.style.zIndex = "1"; // Garante que fica em cima
  ...
  const map = L.map(div).setView([39.6, -8], 7); // Centro ajustado
  ...
  function style(feature) {
    // ALTERAÇÃO 2: Tenta encontrar o nome do distrito em várias propriedades comuns
    ...
    // Se info existir, usa a cor do partido. Se não, usa Rosa Choque (Debug)
    const corFinal = info ? getColor(info.partido) : "#ff00ff"; // Rosa = Erro de nome
  ...
  if (distritosGeo && distritosGeo.features) {
     ...
     // Ajusta o zoom automaticamente para ver Portugal inteiro
     try { map.fitBounds(geoLayer.getBounds()); } catch(e) {}
  }
  
  // Força o mapa a redesenhar o tamanho
...
## Column {width=50%}
```{ojs}
// MAPA 2: INTENSIDADE (Versão Corrigida)
mapaIntensidade = {
  const div = document.createElement("div");
  // 1. Altura Fixa (Obrigatório para evitar ecrã branco)
  div.style.height = "500px"; 
  div.style.width = "100%";
  div.style.zIndex = "1";
  ...
  function style(feature) {
    ...
    // Calcular rácio para opacidade (0.1 a 0.9)
    let ratio = (votos / total) * 3; // O *3 aumenta o contraste
...
sunburst = {
  // 1. Proteção contra falta de dados
  if (!dadosOK) return md`A carregar visualização...`;

  // 2. CRIAR CONTENTOR (O Segredo para aparecer!)
  const div = document.createElement("div");
  div.style.width = "100%";
  div.style.height = "500px"; // Altura fixa obrigatória
  div.style.display = "flex";
  div.style.justifyContent = "center"; // Centra o gráfico
  ...
  const svg = d3.create("svg")
    ...
    .attr("width", "100%")      // Ocupa o div
    .attr("height", "100%")     // Ocupa o div
    .style("max-width", "500px") // Impede que fique gigante
  ...
  // 6. Colocar o SVG dentro do Div e devolver o Div
