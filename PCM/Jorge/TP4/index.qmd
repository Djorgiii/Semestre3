...
sunburst = {
  ...
  // Filtra partidos muito pequenos para "Outros" para limpar o gráfico
  const grandes = sunburstVotosNac.filter(x => x.votos/total >= 0.015); // Ajustei para 1.5%
  ...
  // 6. DESENHAR OS GRUPOS (Arco + Texto)
  // MUDANÇA PRINCIPAL: Criamos um grupo 'g' para cada nó
  const cell = svg.append("g")
    .selectAll("g")
    .data(root.descendants().slice(1)) // Ignora a raiz "Portugal"
    .join("g");

  // Desenha o caminho (a cor)
  cell.append("path")
      .attr("fill", d => { 
          let current = d;
  ...
  cell.append("text")
      .attr("transform", d => {
          // Calcula a posição central e a rotação para o texto
      ...
      // Lógica para esconder texto se o arco for muito pequeno
      .style("opacity", d => (d.x1 - d.x0 > 0.05) ? 1 : 0) 
      .style("pointer-events", "none") // O clique passa para o arco
      .style("fill", "#fff") // Texto branco para contraste
      .style("font-weight", "bold")
      .style("text-shadow", "1px 1px 2px #000") // Sombra para legibilidade
      .text(d.data.name);
